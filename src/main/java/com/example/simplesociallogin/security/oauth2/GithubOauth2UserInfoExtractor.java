...
public class GithubOauth2UserInfoExtractor implements Oauth2UserInfoExtractor {
    ...
    public CustomUserDetails extractUserInfo(OAuth2User oAuth2User) {
        ...
        customUserDetails.setUsername(retrieveAttribute("login",oAuth2User));
        customUserDetails.setName(retrieveAttribute("name",oAuth2User));
        customUserDetails.setEmail(retrieveAttribute("email", oAuth2User));
        customUserDetails.setAvatarUrl(retrieveAttribute("avatar_url", oAuth2User));
        customUserDetails.setProvider(Oauth2Provider.GITHUB);
        customUserDetails.setAttributes(oAuth2User.getAttributes());
        customUserDetails.setAuthorities(Collections.singletonList(new SimpleGrantedAuthority(Oauth2Role.USER.name())));
